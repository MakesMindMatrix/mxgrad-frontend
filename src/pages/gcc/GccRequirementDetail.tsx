...
export default function GccRequirementDetail() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (form.timeline_start && form.timeline_start < today) {
      setSaveError('Timeline start must be today or a future date.');
      return;
    }
    if (form.timeline_start && form.timeline_end && form.timeline_end < form.timeline_start) {
      setSaveError('Timeline end must be on or after the start date.');
      return;
    }
    setSaving(true);
    try {
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    id="detail-timeline_start"
                    type="date"
                    min={todayYYYYMMDD()}
                    value={form.timeline_start}
                    onChange={(e) => setForm((f) => f ? { ...f, timeline_start: e.target.value } : f)}
                  />
                ...
                <div>
                  ...
                  <Input
                    id="detail-timeline_end"
                    type="date"
                    min={form.timeline_start || todayYYYYMMDD()}
                    value={form.timeline_end}
                    onChange={(e) => setForm((f) => f ? { ...f, timeline_end: e.target.value } : f)}
                  />
